...
%
% Draft skeleton produced by Phase P2 (editorial polish, pre-drafting).
%
% The abstract and Section 1 (Introduction) are fully drafted under
% the P2 rules of PRE_DRAFT_TASKS.md / NOVELTY_AND_POSITIONING.md:
%   - 4 contributions, no more.
%   - No 600-cell, no \varphi numerology, no "47 theorems", no
%     "immediate crypto applications", no universal {M_n,O_n} claim
%     outside the Conjecture.
%   - Section 1.4 is exactly two paragraphs (sibling cross-refs).
%   - Each main theorem appears once informally in §1.3 and once
%     formally in its dedicated section.
%
% Sections 2..12 are stubbed with section/subsection headers, tier
% labels, and pointers to the certified JSONs. They are filled in
% during the actual write-up phase.
...
We complement these results by an exhaustive computational
verification of a stabilizer dichotomy $\{M_n, O_n\}$ for
$5 \le n \le 13$, and by a certified reproducibility package whose
...
$\Prb, \Pcs \in \{0,1\}^{(\sqrt{n}\sqrt{n}) \times n}$ be the
projectors that average over row-bands and column-stacks respectively.
...
\subsection{Symbol relabelings $\gamma \in S_n$}
...
\subsection{Centered matrix $\Egamma = \gamma(L) - \tfrac{n+1}{2}J_n$}
...
\subsection{The standard subspace $\Vstdn$ and certificate vectors}
...
collected in
\href{../paper/certified/SIBLING_NOTATION.md}{\texttt{certified/SIBLING\_NOTATION.md}}.
...
\href{../paper/certified/}{\texttt{certified/}}
combines:
...
% =====================================================================
\section{Odd-rank relabelings of base$_1$}\label{sec:base1}
% =====================================================================
...
\subsection{The Sudoku base \texttt{base1}}\label{sec:base1-grid}
...
\href{../paper/certified/recover_base1_22_perms.py}{\texttt{recover\_base1\_22\_perms.py}}
and stored under the input field of
\texttt{base1\_odd\_rank\_22.json}.

\subsection{Exhaustive $S_9$ scan}\label{sec:base1-scan}
...
in the \texttt{outputs.odd\_perms\_full\_list} field of
\texttt{base1\_odd\_rank\_22.json}.
\end{theorem}
...
\begin{enumerate}[label=(\roman*),leftmargin=*]
   ...
   \item \textbf{Integer Smith normal form} of $\Ehat$, yielding
   elementary divisors $(1,1,1,1,1,1,1,0)$ and confirming that the
   cokernel is torsion-free of rank $1$;
   \item \textbf{$\mathbb{F}_2$-Smith normal form} of the reduction of
   $\Ehat$ modulo $2$, agreeing with the integer rank on every
   relabeling.
\end{enumerate}
The four routes are independent in the sense that no two of them
share a common code path past the construction of $\Ehat$. Their
agreement on the $22$-element list rules out single-bug, single-route,
and single-prime artefacts. The full list of certified relabelings is
reproduced in Appendix~\ref{app:22-perms}.
...
(i) the box constraint does not suppress the odd-rank phenomenon at
$n = 6$, and (ii) at $n = 7$ the symmetry group of the simplest
odd-rank certificate is dihedral of order $12$ and embeds inside the
box automorphism group $B_3 < S_7$.

\subsection{Roku-Doku at $n = 6$: $\mathbb{F}_2$-rank census}\label{sec:n6}
...
\subsection{The $n = 7$ group $H_{V_1} \cong D_6$}\label{sec:n7}
...
\href{../../scripts/check_b3_subgroup.py}{\texttt{check\_b3\_subgroup.py}}.
\end{proof}
...
even-coranck (odd-rank) $\Ehat$, and $n = 6$ Roku-Doku already
contains rank-$\le 4$ witnesses. We do not reprove the falsification
in this paper; the existence of explicit counterexamples is the
content of the working draft \cite{HAN-RankParity}, which the present
paper takes as input. From the existence of odd-rank
relabelings we proceed to their algebraic structure
(\S\S\ref{sec:certificates}--\ref{sec:autLM19}).

% =====================================================================
\section{Certificates in $\Vstdn$}\label{sec:certificates}
% =====================================================================
...
% =====================================================================
\section{Orbit structure and $A_8$ saturation}\label{sec:orbits}
% =====================================================================

We now restrict to $n = 9$ and analyze the $G^*$-orbits of the kernel
certificates of \S\ref{sec:cert-taxonomy}. The notation $G^* < S_n$
denotes the closure of the template-action group of
\cite[\S S9w]{HAN-DetDivisibility}, normalized so that
$\one \in S_n$ acts trivially and so that $G^*$ stabilizes
$A_{n-1}$.
...
\href{../../scripts/phase_9_18_S9q_beta_lattice.py}{\texttt{phase\_9\_18\_S9q\_beta\_lattice.py}};
...
\subsection{The $\{M, O_6\}$ dichotomy at $n = 9$}\label{sec:dichotomy-n9}
...
% =====================================================================
\section{The $M_{19}$ laboratory and Hessian symmetry}\label{sec:M19}
% =====================================================================
...
\texttt{M19\_audit.json} and \texttt{M19\_KL\_KR\_3sq\_Q8.json}; it
...
\subsection{Exhaustive $S_9$ audit of $M_{19}$}\label{sec:M19-audit}
...
\subsection{Single-coset structure of the Hessian class $\GMninetn$}
\label{sec:M19-coset}
...
\subsection{Hessian identification: $\KL \cong \KR \cong 3^2{:}Q_8$}
...
intersection is trivial), but they are conjugate via the structural
permutation $\pi_0 \in S_9$ that exchanges the row and column roles
of $M_{19}$.
\end{theorem}
...
The permutation $\pi_0$ relating $\KL$ and $\KR$ is the row-column
exchange of $M_{19}$ acting on the symbol set, not on the row or
column indices. Concretely, $\pi_0$ is one of the eight generators
listed in \texttt{outputs.K\_L\_generators\_perms}; its outer
conjugation action on $\KL$ produces $\KR$.
\end{remark}
...
% =====================================================================
\section{Stabilizer dichotomy $\{M_n, O_n\}$}\label{sec:dichotomy}
% =====================================================================
...
\subsection{Exhaustive verification at $5 \le n \le 13$}
...
\href{../../scripts/phase_9_18_S9q_gamma1.py}{\texttt{phase\_9\_18\_S9q\_gamma1.py}};
total wall-time $\approx 31$ seconds. The certified output is
\texttt{stabilizers\_5\_to\_13.json}.

\subsection{Analytic identification at $n \in \{7, 9, 11\}$}
...
\subsection{General-$n$ conjecture}\label{sec:dichotomy-general}
...
\subsection{Triviality of $\Aut(L_{M_{19}})$}\label{sec:aut-triv}
...
\subsection{Box / $A_8$ origin of $\KL$}\label{sec:KL-origin}
...
\href{../paper/certified/TIER_LEDGER.md}{\texttt{certified/TIER\_LEDGER.md}}.
...
\href{../paper/certified/}{\texttt{certified/}} containing
the inputs, outputs, source-data hashes, producer-script hashes,
seed, UTC timestamp, paper label, and tier. The eleven JSONs are
pinned by
\href{../paper/certified/MANIFEST.sha256}{\texttt{MANIFEST.sha256}}.
The verifier
\href{../paper/certified/verify_all.py}{\texttt{verify\_all.py}}
re-checks the manifest and the claim-level invariants of each JSON;
the regenerator
\href{../paper/certified/reproduce_all.py}{\texttt{reproduce\_all.py}}
...
\begin{enumerate}[label=(O\arabic*),leftmargin=*]
   \item \textbf{Cokernel structure of $\Ehat$.} The Smith normal form
   of $\Ehat$ at odd-rank $\gamma$ on \texttt{base1} is
   $(1, \dots, 1, 0)$ (rank $7$, free cokernel of rank $1$). Whether
   the cokernel is structured uniformly across the $22$ relabelings,
   or across $M_{19}$'s $100$ relabelings, is open.
...
\section{Exhaustive base$_1$ odd-rank list}\label{app:22-perms}
...
\section{$M_{19}$: 72 generators of $\KL$ and presentation of $\HesGroup$}
...
\section{Verified-range stabilizer table for $n \in \{5, \dots, 13\}$}
...
\begin{itemize}[leftmargin=*]
   ...
   \href{../paper/certified/TIER_LEDGER.md}{\texttt{TIER\_LEDGER.md}};
   ...
   \href{../paper/certified/UNIVERSALITY_LOCK.md}{\texttt{UNIVERSALITY\_LOCK.md}};
   \item the sibling-notation reconciliation
   \href{../paper/certified/SIBLING_NOTATION.md}{\texttt{SIBLING\_NOTATION.md}}.
\end{itemize}
