...
Figure~\ref{fig:sudoku-projectors} visualizes the identity: the row-band
projector $\Prb$ and the column-stack projector $\Pcs$, overlaid on a basis
adapted to the $3\times3$ box decomposition, on which
$\Prb\Egamma\Pcs^{\!\top}=0$.

\begin{figure}[t]
   \centering
   \includegraphics[width=0.7\textwidth]{fig1_sudoku_projectors.pdf}
   \caption{Row-band ($\Prb$) and column-stack ($\Pcs$) projector overlays
   on a basis adapted to the $3\times3$ box decomposition, illustrating the
   Box Band Lemma identity $\Prb\Egamma\Pcs^{\!\top}=0$
   (Theorem~\ref{thm:box-band}).}
   \label{fig:sudoku-projectors}
\end{figure}
...
a tier convention defined for this paper (recorded in the tier ledger below). The composite tier
