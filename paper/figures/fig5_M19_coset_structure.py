...
================================
This paper ?8 -- Bilateral coset structure of $\Gamma^*_{M_{19}}$.

The structural facts depicted (all read from ``certified/M19_audit.json``
and ``certified/M19_KL_KR_affine_D4.json``):

* $|\Gamma^*_{M_{19}}| = 72$ (one bilateral coset);
* $\Gamma^*_{M_{19}} = K_L\,\pi_0 = \pi_0\,K_R$, where
...
* the ambient coordinate stabilizer of the value-multiplicity vector
    $w_9^L \in \mathbb{Z}^9$ is $S_3 \times S_6$ of order $4320$;
    it is a separate object, and $K_L$, $K_R$ are not subgroups of it.
...
Source: ``certified/M19_audit.json`` and ``certified/M19_KL_KR_affine_D4.json``.
"""
...
def main() -> Path:
    ...
    klkr = _load("M19_KL_KR_affine_D4.json")
    ...
    ambient = int(out_a.get("ambient_coordinate_stabilizer_S_9_of_shared_left_kernel_size", 4320))
    KL_eq_KR = bool(out_k.get("K_L_eq_K_R", False))
    Y2_pi0 = bool(out_k.get("Y2_conjugate_via_pi0", True))
    pi0 = out_k.get("pi0_perm")
    KL_in_ambient = bool(
        out_k.get("K_L_affine_structure", {}).get(
            "is_subgroup_of_ambient_coordinate_stabilizer", False
        )
    )
    KR_in_ambient = bool(
        out_k.get("K_R_affine_structure", {}).get(
            "is_subgroup_of_ambient_coordinate_stabilizer", False
        )
    )
    ...
    assert not KL_in_ambient and not KR_in_ambient, (
        "figure expects K_L and K_R to be separate from Stab_S9(w9L)"
    )
    ...
    # ---- Separate label: ambient coordinate stabilizer S_3 x S_6 (4320). ----
    ax.add_patch(mpatches.Rectangle(
        (0.9, 4.75), 8.2, 0.75,
        linewidth=1.0, edgecolor="0.35", facecolor="0.96"
    ))
    ax.text(
        5.0, 5.12,
        rf"Separate ambient stabilizer:  $\mathrm{{Stab}}_{{S_9}}(w_9^L) "
        rf"\cong S_3 \times S_6$  (order ${ambient}$)" "\n"
        rf"$K_L,K_R \nleq \mathrm{{Stab}}_{{S_9}}(w_9^L)$; separate from the coset",
    ...
    ax.text(
        5.0, 3.65,
        rf"$\Gamma^*_{{M_{{19}}}} = K_L\,\pi_0 = \pi_0\,K_R$, "
    ...
    ax.set_title(
        r"Bilateral coset structure $\Gamma^*_{M_{19}} = "
        r"K_L\,\pi_0 = \pi_0\,K_R$",
        fontsize=10.5,
    )
