...
| Symbol | Sibling (Det.-divisibility) | This paper | Same object? | Notes |
|---|---|---|---|---|
...
| $m$ | $m = (n+1)/2$ | $m_n = (n+1)/2$ | **Yes** | Subscript added in this paper for clarity at variable $n$. |
| $J$ | $n \times n$ all-ones matrix | $J_n$ | **Yes** | Same. |
| $E$ | $E = L - mJ$ (centered matrix) | $E_{\gamma,L} = \gamma(L) - m_n J_n$ | **Yes** with relabeling | This paper generalizes $E$ by composing with a symbol relabeling $\gamma \in S_n$; setting $\gamma = \mathrm{id}$ recovers the sibling's $E$. Sign convention is identical. |
...
| $G$ | $G = P^\top P = I_{n-1} + J_{n-1}$, $\det(G) = n$ | (used in spirit, not labelled) | **Same identity available** | Both papers exploit $G = I + J$ with $\det(G) = n$. This paper does not need $G$ explicitly because it focuses on $\rank$ / SNF rather than $\det$. |
| Gram-projected matrix | $\widehat{E} = P^\top E P$, called $\Estd = GA$ in the sibling | $\widehat{E}_{\gamma,L} = P_n^\top E_{\gamma,L} P_n$ | **Yes** | Identical formula; this paper carries the $\gamma$ decoration. |
| $A$ (difference matrix) | $A_{ij} = L_{ij} - L_{i,n-1}$, $0 \le i, j \le n-2$, with $\widehat{E} = G A$ | not used | — | This paper does not use the difference-matrix factorization; rank / SNF / kernel statements are made directly on $\widehat{E}_{\gamma,L}$. |
| $\det(\widehat{E}) = n \det(A)$ | Lemma "Gram factorization" (sibling §2) | not invoked | — | This paper does not reduce to determinants. The two results are orthogonal: sibling computes $\det$, this paper computes $\rank$, $\ker$, and the stabilizer / lattice action. |
| $\rank$, $\ker$ | (not central to sibling) | $\rank(E_{\gamma,L}) = \rank(E_{\gamma,L} \mid_{V_{\mathrm{std},n}})$ (since the all-ones direction is always in the kernel) | — | This paper's rank statements are independent of basis and equal to the rank of the Gram-projected $\widehat{E}_{\gamma,L}$ minus zero (no further drop), because $G$ is invertible over $\mathbb{Q}$. |
| SNF | not used | $\SNF(\widehat{E}_{\gamma,L})$ | — | This paper default for SNF is $\widehat{E}_{\gamma,L}$, as locked in [`paper/notation.tex`](../paper/notation.tex). |
| $\Stab$, $\Aut$ | $\Aut$ refers to autotopism in informal language | $\Stab_{G^*}(v)$ for $v \in V_{\mathrm{std},n}$; $\Aut(L)$ = Latin-square autotopism group; $K_L$, $K_R$ = **affine coset-symmetry groups** of $E_{\gamma,L}$ (definitely **not** stabilizers of $w^L_9$, **not** autotopisms) | **Distinct objects** | The sibling does not define $K_L$, $K_R$ — these are purely constructs of this paper. The disambiguation between $K_L / K_R$ vs $\Aut(L_{M_{19}})$ is exactly Certified Proposition G of this paper. |

---

## B. Disambiguation flags (this paper must avoid implicit identification)

1. **Field choice.** This paper uses $\mathbb{Q}$ throughout; the sibling
   uses $\mathbb{R}$. For all integer-matrix statements (rank, kernel,
   SNF), the choice is immaterial. This paper must **not** invoke any
   real-analytic or topological argument (orthogonal projection,
   eigenvalue inequality, etc.) without explicitly switching to
   $\mathbb{R}$.

2. **Indexing.** This paper indexes $1, \dots, n$; sibling indexes
   $0, \dots, n-1$. When formulas are quoted across papers, the index
   shift must be applied. The basis vector $b_i$ in the sibling
   corresponds to $e_{i+1} - e_n$ in this paper (column $i+1$ minus last).
   The matrix $P$ is the same matrix once indices are aligned.

3. **$E$ vs $E_\gamma$.** The sibling's $E$ is this paper's
   $E_{\mathrm{id}, L}$. When citing the sibling's results inside
   this paper, the $\gamma$ subscript must be **explicitly** absent
   ($\gamma = \mathrm{id}$); otherwise the sibling's identity is being
   misapplied.

4. **$\widehat{E}$ vs $\Estd$.** These are the same matrix. This paper
...
5. **$K_L$, $K_R$ vs $\Aut(L)$.** These are **different** groups in
   ...
   the affine coset symmetry of the bilateral coset structure of
     $\Gamma^*_{M_{19}}$ (this paper §8).
   - $\Aut(L)$ is the Latin-square autotopism group of $L$ in the
     classical (Dénes–Keedwell) sense.
   Certified Proposition G proves $|\Aut(L_{M_{19}})| = 1 \neq 72 = |K_L|$,
...
| If the sibling writes … | This paper writes … |
|---|---|
...
- [x] Same definition of $E$ (this paper generalizes by $\gamma$).
...
- [x] Sign conventions agree ($m = (n+1)/2$, $E = L - mJ$, $E\one = 0$,
  $\one^\top E = 0$).
- [x] No symbol collision with sibling's $A$ (this paper uses $A_{n-1}$
