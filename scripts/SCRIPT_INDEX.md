...
| `phase_9_14.py`                       | $n=6$ $\mathbb{F}_2$-rank parity proxy: counts of grids $L$ with $\rank_{\mathbb{F}_2} \widehat{E}_{\mathrm{id},L} = n-1$ for Sudoku-6 vs Latin-Square-6, first-row-fixed; produces the two-proportion $z$-statistic. (NB: this is the $\mathbb{F}_2$ rank-parity proxy, not an integer odd-rank census.) | `phase_9_14_results.json` |
...
| `phase_9_18_S9x_bis.py`               | Group-invariants and $K_L$/$K_R$ coset audit at $L_{M_{19}}$: subgroup membership, block systems, $K_L \cap K_R$ size, conjugation $K_L = \pi_0 K_R \pi_0^{-1}$ via the lex-min external $\pi_0 \in \Gamma^*_{M_{19}}$. | `phase_9_18_S9x_bis_results.json` |
